...
func SimpleNew() (*Tracee, error) {
	...
	if api_ver != 12 {
		return nil, fmt.Errorf("Got KVM api version %d, expected %d\n", api_ver, 12)
	}
	...
	if _, _, errno := syscall.Syscall(syscall.SYS_IOCTL, uintptr(sys_fd), getCPUID, uintptr(unsafe.Pointer(i))); errno != 0 {
		return nil, fmt.Errorf("Check CPUID entries err %v", errno)
	}
	Debug("%v", i)
	...
	if _, _, errno := syscall.Syscall(syscall.SYS_IOCTL, uintptr(vcpufd), setCPUID, uintptr(unsafe.Pointer(i))); errno != 0 {
		return nil, fmt.Errorf("Set  CPUID entries err %v", errno)
	}
	...
	if _, err := tioctl(vcpufd, setGuestDebug, uintptr(unsafe.Pointer(&debug[0]))); err != nil {
		return nil, fmt.Errorf("Setting guest debug")
	}
	...
	case ExitHlt:
		Debug("EXITHLT!\n")
	default:
		return nil, fmt.Errorf("Got exit_reason %d,expected KVM_EXIT_HLT (%d)\n", vmrun.ExitReason, ExitHlt)
